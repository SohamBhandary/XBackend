...
import com.Soham.X.Response.PaymentLinkResponse;
...
public class PaymentController {
    ...
    public ResponseEntity<PaymentLinkResponse> createSubcribtion
            (@PathVariable String planType, @RequestHeader ("Authorization") String jwt) throws UserException, RazorpayException {
    ...
    public ResponseEntity<String> fetchPaymetn(@PathVariable String paymentLinkId) throws RazorpayException {
        ...
        try {
            ...
            if(payment.get("status").equals("paid")) {
                user.getVerfication().setStartedAt(LocalDateTime.now());
                user.getVerfication().setPlanType(plan);

                if (plan.equals("yearly")) {
                    LocalDateTime endsAt = user.getVerfication().getStartedAt().plusYears(1);
                    user.getVerfication().setEndsAt(endsAt);
                }
                else if (plan.equals("monthly")) {
                    LocalDateTime endsAt = user.getVerfication().getStartedAt().plusMonths(1);
                    user.getVerfication().setEndsAt(endsAt);
                }
